...
# Carregar o dataset
df = pd.read_csv('datasets/dataset-1000-com-preco-promocional-e-renovacao-estoque.csv')
...
# Remover a coluna 'FLAG_PROMOCAO'
df = df.drop(columns=['FLAG_PROMOCAO'])
...
features = ['ID_PRODUTO', 'PRECO', 'ANO', 'MES', 'DIA']
dadospromo = df[features]

# Fazer previsões para o dataset Promoção
previsoes = model.predict(dadospromo)
...
previsoes_promo['PREVISAO_QUANTIDADE_ESTOQUE'] = previsoes_promo['PREVISAO_QUANTIDADE_ESTOQUE'].astype(int)

previsoes_promo.to_csv('previsoes_promo.csv', index=False)